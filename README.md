...
`SubProcess` takes a list of strings and handles the quoting internally. By default the standard output and standard error are captured into strings. By default the standard output and standard error are captured into strings, accessible as `.OutputString` and `ErrorString` after the sub-process has started.
